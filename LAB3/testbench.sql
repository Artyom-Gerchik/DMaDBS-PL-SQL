...
BEGIN
    IF GET_PACKAGE_TEXT('DEVELOPER', 'PAKETIK',
                        'PACKAGE', 'PAKETIK')
        !=
       GET_PACKAGE_TEXT('PRODUCTOR', 'PAKETIK',
                        'PACKAGE', 'PAKETIK')
    THEN
        ADD_FUNCTION_OR_PROCEDURE('DEVELOPER', 'PAKETIK', 'PACKAGE');
    END IF;
END;
...
FROM ALL_SOURCE
WHERE OWNER = UPPER('DEVELOPER')
  AND NAME = UPPER('PAKETIK')
  AND TYPE = UPPER('PACKAGE')
  AND TEXT != chr(10);

BEGIN
    DBMS_OUTPUT.PUT_LINE(GET_PACKAGE_TEXT('DEVELOPER', 'PAKETIK', 'PACAKGE'));
END;
...
SELECT * FROM ALL_CONSTRAINTS WHERE OWNER = 'DEVELOPER'


SELECT *
FROM (SELECT OWNER, CONSTRAINT_NAME, DELETE_RULE
      FROM ALL_CONSTRAINTS
      WHERE OWNER = UPPER('DEVELOPER')
        AND TABLE_NAME = UPPER('PERSONS')
        AND CONSTRAINT_TYPE = 'R') DEVELOPER
         FULL OUTER JOIN
     (SELECT OWNER, CONSTRAINT_NAME, DELETE_RULE
      FROM ALL_CONSTRAINTS
      WHERE OWNER = UPPER('PRODUCTOR')
        AND TABLE_NAME = UPPER('PERSONS')
        AND CONSTRAINT_TYPE = 'R') PRODUCTOR
     ON DEVELOPER.CONSTRAINT_NAME = PRODUCTOR.CONSTRAINT_NAME;