    ...
    FOR RECORD IN TABLE_NAMES
        LOOP
            IF RECORD.PRODUCTOR_TABLE_NAME IS NULL THEN
                ...
                DBMS_OUTPUT.PUT_LINE('CREATE OR REPLACE TABLE ' || RECORD.DEVELOPER_TABLE_NAME ||
                                     ' # IN PRODUCTOR SCHEMA.');
                CONTINUE;
            END IF;
            IF RECORD.DEVELOPER_TABLE_NAME IS NULL THEN
                DBMS_OUTPUT.PUT_LINE('DROP TABLE ' || RECORD.PRODUCTOR_TABLE_NAME || ';' || ' IN PRODUCTOR SCHEMA.');
                CONTINUE;
            END IF;
            ...
            CHECK_FOREIGN_KEYS(DEV_SCHEMA_NAME, PROD_SCHEMA_NAME, RECORD.DEVELOPER_TABLE_NAME);
    ...
    FOR RECORD IN TABLE_COLUMNS
        LOOP
            IF RECORD.DEVELOPER_COLUMN_NAME IS NULL THEN
                ...
                SELECT DATA_TYPE TMP
                INTO TMP
                FROM ALL_TAB_COLUMNS
                WHERE OWNER = UPPER(PROD_SCHEMA_NAME)
                  AND COLUMN_NAME = UPPER(RECORD.PRODUCTOR_COLUMN_NAME);
                DBMS_OUTPUT.PUT_LINE('ALTER TABLE ' || TABLE_NAME_TO_CHECK || ' ADD ' ||
                                     RECORD.PRODUCTOR_COLUMN_NAME || ' ' || TMP || '; IN DEVELOPER.');
                CONTINUE;
            END IF;
            IF RECORD.PRODUCTOR_COLUMN_NAME IS NULL THEN
                ...
                FROM ALL_TAB_COLUMNS
                WHERE OWNER = UPPER(DEV_SCHEMA_NAME)
                  AND COLUMN_NAME = UPPER(RECORD.DEVELOPER_COLUMN_NAME);
                DBMS_OUTPUT.PUT_LINE('ALTER TABLE ' || TABLE_NAME_TO_CHECK || ' ADD ' ||
                                     RECORD.DEVELOPER_COLUMN_NAME || ' ' || TMP || '; IN PRODUCTOR.');
                CONTINUE;
            END IF;
    ...
    FOR RECORD IN COLUMN_DATA_TYPES
        LOOP
            ...
            ELSE
                ...
                DBMS_OUTPUT.PUT_LINE('ALTER TABLE ' || TABLE_NAME_FOR_OUTPUT || ' MODIFY( ' ||
                                     COLUMN_NAME_TO_CHECK || ' ' || RECORD.DEVELOPER_COLUMN_DATA_TYPE || ' );');
                TMP := 1;
            END IF;
        END LOOP;
...
BEGIN
    FOR RECORD IN TABLE_CONSTRAINTS_NAMES
        LOOP
            IF RECORD.DEVELOPER_CONSTRAINT_NAME IS NULL THEN
                DBMS_OUTPUT.PUT_LINE('ALTER TABLE ' || UPPER(TABLE_NAME_TO_CHECK) || CHR(10)
                    || 'DROP CONSTRAINT ' || RECORD.PRODUCTOR_CONSTRAINT_NAME || ';');
                CONTINUE;
            END IF;
            IF RECORD.PRODUCTOR_CONSTRAINT_NAME IS NULL THEN
                DBMS_OUTPUT.PUT_LINE('ALTER TABLE ' || UPPER(TABLE_NAME_TO_CHECK) || CHR(10)
                    || 'ADD ' || RECORD.DEVELOPER_CONSTRAINT_NAME || ';');
                CONTINUE;
            END IF;
            ...
            ELSE
                ...
                DBMS_OUTPUT.PUT_LINE('ALTER TABLE ' || TABLE_NAME_TO_CHECK || ' DROP CONSTRAINT ' ||
                                     RECORD.PRODUCTOR_CONSTRAINT_NAME || '; IN PRODUCTOR.');
                DBMS_OUTPUT.PUT_LINE('ALTER TABLE ' || TABLE_NAME_TO_CHECK || ' ADD ' ||
                                     RECORD.DEVELOPER_CONSTRAINT_NAME || '; IN PRODUCTOR.');
        ...
        LOOP
            ...
            IF GET_INDEX_TEXT(DEV_SCHEMA_NAME, RECORD.DEVELOPER_INDEX_NAME)
               ...
               GET_INDEX_TEXT(PROD_SCHEMA_NAME, RECORD.PRODUCTOR_INDEX_NAME)
                ...
                IF RECORD.DEVELOPER_UNIQUENESS = RECORD.PRODUCTOR_UNIQUENESS AND TMP = 999 THEN
                    TMP := 1;
                END IF;
                IF RECORD.DEVELOPER_UNIQUENESS != RECORD.PRODUCTOR_UNIQUENESS AND TMP = 1 THEN
                    DBMS_OUTPUT.PUT_LINE('DROP INDEX ' || RECORD.PRODUCTOR_INDEX_NAME || '; IN PRODUCTOR.');
...
CREATE OR REPLACE PROCEDURE CHECK_PACKAGES(DEV_SCHEMA_NAME VARCHAR2,
                                           PROD_SCHEMA_NAME VARCHAR2)
    IS
    CURSOR PACKAGES_NAMES IS
        SELECT *
        FROM (SELECT OBJECT_NAME DEVELOPER_NAME
              FROM ALL_OBJECTS
              WHERE OWNER = UPPER(DEV_SCHEMA_NAME)
                AND OBJECT_TYPE = 'PACKAGE') DEVELOPER
                 FULL OUTER JOIN
             (SELECT OBJECT_NAME PRODUCTOR_NAME
              FROM ALL_OBJECTS
              WHERE OWNER = UPPER(PROD_SCHEMA_NAME)
                AND OBJECT_TYPE = 'PACKAGE') PRODUCTOR
             ON DEVELOPER.DEVELOPER_NAME = PRODUCTOR.PRODUCTOR_NAME;
BEGIN
    FOR RECORD IN PACKAGES_NAMES
        LOOP
            IF RECORD.PRODUCTOR_NAME IS NULL THEN
                ADD_FUNCTION_OR_PROCEDURE(DEV_SCHEMA_NAME, RECORD.DEVELOPER_NAME, 'PACKAGE');
                CONTINUE;
            END IF;
            IF RECORD.DEVELOPER_NAME IS NULL THEN
                DBMS_OUTPUT.PUT_LINE('DROP PACKAGE ' || RECORD.PRODUCTOR_NAME || '; IN PRODUCTOR.');
                CONTINUE;
            END IF;

            IF GET_PACKAGE_TEXT(DEV_SCHEMA_NAME, RECORD.DEVELOPER_NAME,
                                'PACKAGE')
                !=
               GET_PACKAGE_TEXT(PROD_SCHEMA_NAME, RECORD.PRODUCTOR_NAME,
                                'PACKAGE')
            THEN
                ADD_FUNCTION_OR_PROCEDURE(DEV_SCHEMA_NAME, RECORD.DEVELOPER_NAME, 'PACKAGE');
            END IF;
...
CREATE OR REPLACE FUNCTION GET_PACKAGE_TEXT(SCHEMA_NAME VARCHAR2,
    ...
    FLAG BOOLEAN         := FALSE;
BEGIN
    FOR RECORD IN GET_TEXT
        LOOP
            IF REGEXP_LIKE(TEXT, 'END ' || UPPER(PACKAGE_NAME) || ';?$') THEN
                EXIT;
            END IF;
            TEXT := TEXT || RECORD.OBJECT_TEXT;
        END LOOP;
    RETURN TEXT;
END GET_PACKAGE_TEXT;


CREATE OR REPLACE PROCEDURE CHECK_FOREIGN_KEYS(DEV_SCHEMA_NAME VARCHAR2, PROD_SCHEMA_NAME VARCHAR2,
                                               TABLE_NAME_TO_CHECK VARCHAR2)
    IS
    CURSOR FOREIGN_KEYS IS
        SELECT *
        FROM (SELECT CONSTRAINT_NAME DEVELOPER_NAME
              FROM ALL_CONSTRAINTS
              WHERE OWNER = UPPER(DEV_SCHEMA_NAME)
                AND TABLE_NAME = UPPER(TABLE_NAME_TO_CHECK)
                AND CONSTRAINT_TYPE = 'R') DEVELOPER
                 FULL OUTER JOIN
             (SELECT CONSTRAINT_NAME PRODUCTOR_NAME
              FROM ALL_CONSTRAINTS
              WHERE OWNER = UPPER(PROD_SCHEMA_NAME)
                AND TABLE_NAME = UPPER(TABLE_NAME_TO_CHECK)
                AND CONSTRAINT_TYPE = 'R') PRODUCTOR
             ON DEVELOPER.DEVELOPER_NAME = PRODUCTOR.PRODUCTOR_NAME;
BEGIN
    FOR RECORD IN FOREIGN_KEYS
        LOOP
            IF RECORD.DEVELOPER_NAME IS NULL THEN
                DBMS_OUTPUT.PUT_LINE('ALTER TABLE ' || UPPER(TABLE_NAME_TO_CHECK) || CHR(10)
                    || 'DROP CONSTRAINT ' || RECORD.PRODUCTOR_NAME || ';');
                CONTINUE;
            END IF;
